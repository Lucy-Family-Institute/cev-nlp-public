...
path = DIR_DATA + "input/despojo/etiquetado_res_2021-07-07.xlsx"
outpath = DIR_DATA + "output/despojo"
os.makedirs(outpath, exist_ok=True)

outfiles = os.listdir(outpath)
if "load_source.p" not in outfiles:
    ls = LoadSource(
        db_name="etiquetas_despojo",
        db_coll="document",
        path=path,
    )
...
for type_ in types:
    bs.draw_frequent_words(
        type_,
        for_each_doc=False,
        path=os.path.join(outpath, f"despojo_freqs_{type_}.pdf"),
        topn=35,
    )
...
for pair in pairs:
    ...
    bs.draw_heatmaps(
        *pair,
        for_each_doc=False,
        path=os.path.join(
            outpath, f"despojo_heatmap_{pair[0]}_vs_{pair[1]}.pdf"
        ),
        graphs=cooc,
        topn=35,
    )
    ...
    bs.draw_graphs(
        *pair,
        for_each_doc=False,
        path=os.path.join(outpath, f"despojo_graph_{pair[0]}_vs_{pair[1]}.html"),
        graphs=cooc,
        max_conns=50,
        plotly=False,
    )
