...
path = DIR_DATA + "input/tierras-narino/"
outpath = DIR_DATA + "output/sentencias_narino"
...
for type_ in types:
    bs.draw_frequent_words(
        type_,
        for_each_doc=False,
        path=os.path.join(outpath, f"sentencias_narino_freqs_{type_}.pdf"),
        topn=35,
    )
...
for pair in pairs:
    ...
    bs.draw_heatmaps(
        *pair,
        for_each_doc=False,
        path=os.path.join(
            outpath, f"sentencias_narino_heatmap_{pair[0]}_vs_{pair[1]}.pdf"
        ),
        graphs=cooc,
        topn=35,
    )
    ...
    bs.draw_graphs(
        *pair,
        for_each_doc=False,
        path=os.path.join(outpath, f"sentencias_narino_graph_{pair[0]}_vs_{pair[1]}.html"),
        graphs=cooc,
        max_conns=50,
        plotly=False,
    )
