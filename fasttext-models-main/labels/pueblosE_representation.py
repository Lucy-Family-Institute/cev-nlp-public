...
# Get etiquetas from N7 Pueblos étnicos:
chunksize = 10 ** 5
...
with pd.read_csv(labels_path, sep="¬", compression="gzip", usecols=usecols, chunksize=chunksize) as reader:
    for chunk in reader:
        chunk = chunk[chunk.label.str.contains("Relaciones interétnicas")]
        chunks.append(chunk.drop_duplicates(subset="text"))
df = pd.concat(chunks).drop_duplicates(subset="text")
print("N7 Pueblos étnicos dataframe has shape", df.shape)
...
df.to_excel("pueblosE_vec_representation.xlsx", index=False)
...
average_df.to_excel("pueblosE_vec_representation_average.xlsx", index=False)
