...
class LoadSource(object):
    def __init__(
        ...
        ----------
        path: Path
            Path where files are stored. It can be the path to a file or a directory.
        db_name: str
    ...
    def load_sentencias(
        ...
        all_files = glob.glob(self.path + extension_pattern)
    ...
    def load_files_labels(self):
        file_name = self.path
        df_labels = pd.read_excel(file_name)
        df_text = df_labels[["texto_marcado", "codigo_entrevista"]].drop_duplicates()
        df_text = df_text.set_index("codigo_entrevista")
        print("Load files :", df_text.shape)
        count = 0
        for index, row in df_text.iterrows():
            if index in self.filter_files or len(self.filter_files) == 0:
                single_file = index
                count += 1
                name = single_file.replace(" ", "")
                pdf = row["texto_marcado"]

                single_dict = {'path': single_file, 'file_name': name,
                               'date': None,
                               'year': None, 'month': None, 'day': None, 'num_pages': 1,
                               'pages_raw': list([pdf]), 'text_raw': pdf, 'sentences': [], 'parsed_doc': [],
                               "identifier": single_file}
    ...
    def ocr_extract_text(file_path):
        list_page = []
        # dir /rep/temp/ is chosen to run in the server only
        with tempfile.TemporaryDirectory(dir="/rep/temp/") as path:
            pages = convert_from_path(file_path, 500, output_folder=path)
    ...
    def load_files_entrevistas(self):
        ...
        usecols = ["interview_id", "fecha_inicio", "text"]
        if self.keyword is not None:
            usecols.append(self.keyword)
        ...
        if self.keyword is not None:
            ...
            df_text = df_interviews.drop_duplicates()
        del df_interviews
        # resetting index
...
if __name__ == "__main__":
    path = "/home/vvargas/repositories/dataset/survey/fichas_interviews.csv.gz"
    ls = LoadSource(
        db_name="entrevistas_masacres",
        db_coll="document",
        path=path,
        keyword="v_m_masacre",
        filter_files=[0, 1],
